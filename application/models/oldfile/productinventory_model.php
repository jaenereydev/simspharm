...
class Productinventory_model extends CI_Model
{   
    //--------------------------------------------------------------------------
     ...
     public function get_history()
    {
        $sql = "SELECT h.date as date, p.p_no as p_no, p.longdesc as longdesc, 
            ...
            (SELECT case 
                        ...
                        END as total
            from producthistory where p_no = p.p_no and description = 'CLASSIFYING' and date = CURDATE()) as CLASSIFYING,
            (SELECT case 
    ...
    public function get_historysearch($date)
    {
        $sql = "SELECT h.date as date, p.p_no as p_no, p.longdesc as longdesc, 
            ...
            (SELECT case 
                        ...
                        END as total
            from producthistory where p_no = p.p_no and description = 'CLASSIFYING' and date = '$date') as CLASSIFYING,
            (SELECT case 
                        when sum(instock) IS NULL THEN 0  
                        else sum(instock)
                        END as total
